...
/**
 * Created by Hu_2015 on 2016/3/19.
 */
...
//    17-20位代表闰月是大月还是小月
     ...
     */
//    构造器
    public GreToLun(String cal1) throws ParseException {
...
//农历年份
        year = iYear;
        yearCyl = iYear - 1864;//1864年为甲子年  60年一轮回
 ...
 */
        int greYear = Integer.parseInt(year);
//        转换
        String bT;
        String cD;
        String TDyear;
        int aT = (greYear - 3) % 1000 % 100 % 10;
        //int aD = (greYear - 3) % 12;
        if (aT == 0)
            aT = 9;
        bT = TG[aT - 1];
    ...
    public static String toTDyear(String cal) throws ParseException {
        //while ()
        String NoLi;
        //System.out.print("公历日期：");
        /*
        Scanner in = new Scanner(System.in);//输入一个日期，在1901-2014年之间  格式为**年**月**日
        cal = in.next();
        */
        //greTolun a;
        GreToLun a = new GreToLun(cal);
        String[] b = a.toString().split("年");
        //System.out.print("农历日期：");

        //GreToLun.greTolun(b[0]);
        //NoLi = GreToLun.greTolun(b[0]).concat("年").concat(b[1]);
